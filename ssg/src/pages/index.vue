...
const init = async () => {
  ...
  const dists = points.value.map((p) =>
    points.value.map((q) => euclideanDistance(p, q))
  );
...
const generateCategoryColors = () => {
  ...
  responseData.value.forEach((element: any) => {
    if (
      !categories.value.find((category) => category.genre === element.genre)
    ) {
...
const getCategoryColor = (category: string) => {
  const categoryIndex = categories.value.findIndex(
    (element) => element.genre === category
  );
...
const drawCanvas = () => {
  ...
  const xExtent = d3.extent(solution.value, (d) =>
    parseFloat(d[dimX] as unknown as string)
  ) as [number, number];
  const yExtent = d3.extent(solution.value, (d) =>
...
const secondaryInit = () => {
  ...
  // This is required for SSG
  // if (typeof window !== "undefined") {
  //   var chart = new ApexCharts(
  //     document.querySelector("#second-chart"),
  //     options
  //   );

  //   chart.render();
  // } else {
  //   consola.error("Window not defined");
  // }
};
