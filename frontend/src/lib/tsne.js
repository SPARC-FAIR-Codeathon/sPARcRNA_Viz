...
var getopt = function (opt, field, defaultval) {
  if (Object.prototype.hasOwnProperty.call(opt, field)) {
    return opt[field]
  } else {
    return defaultval
  }
...
const tSNE = function (optt) {
  let opt = optt || {}
  this.perplexity = getopt(opt, 'perplexity', 30)
  this.dim = getopt(opt, 'dim', 2)
  this.epsilon = getopt(opt, 'epsilon', 10)
  this.iter = 0
}

tSNE.prototype = {
  // ... (copy all the prototype methods from the original file)
  ...
  debugGrad: function () {
    ...
    for (var i = 0; i < N; i++) {
      for (var d = 0; d < this.dim; d++) {
        ...
        console.log(
          i + ',' + d + ': gradcheck analytic: ' + analytic + ' vs. numerical: ' + numerical
        )
